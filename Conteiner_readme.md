...
<table>
  <tr>
    <td align="center" width="50%"><a href="./README.md"><b>📁 Работа с файлами</b></a></td>
    <td align="center" width="50%"><b>📦 Работа с контейнерами (Вы здесь)</b></td>
  </tr>
</table>
...
|---|---|---|---|---|---|---|
| vector | <vector> | Нет | Нет | O(1) | O(n) в середине, O(1) в конце | Непрерывная |
| deque | <deque> | Нет | Нет | O(1) | O(n) в середине, O(1) в начале/конце | Блочная |
| list | <list> | Нет | Нет | O(n) | O(1) везде | Узлы |
| forward_list | <forward_list> | Нет | Нет | O(n) | O(1) после итератора | Узлы |
| array | <array> | Нет | Нет | O(1) | Нельзя | Непрерывная |
| set | <set> | Да | Да | O(log n) | O(log n) | Дерево |
| multiset | <set> | Нет | Да | O(log n) | O(log n) | Дерево |
| map | <map> | Ключи | Ключи | O(log n) | O(log n) | Дерево |
| multimap | <map> | Нет | Ключи | O(log n) | O(log n) | Дерево |
| unordered_set | <unordered_set> | Да | Нет | O(1) ср. | O(1) ср. | Хеш-таблица |
| unordered_multiset | <unordered_set> | Нет | Нет | O(1) ср. | O(1) ср. | Хеш-таблица |
| unordered_map | <unordered_map> | Ключи | Нет | O(1) ср. | O(1) ср. | Хеш-таблица |
| unordered_multimap | <unordered_map> | Нет | Нет | O(1) ср. | O(1) ср. | Хеш-таблица |
| stack | <stack> | — | — | top | push/pop (LIFO) | Адаптер |
| queue | <queue> | — | — | front/back | push/pop (FIFO) | Адаптер |
| priority_queue | <queue> | — | Приоритет | top | push/pop (max/min) | Адаптер |
| span (C++20) | <span> | — | — | O(1) | Не владеет | Представление |
